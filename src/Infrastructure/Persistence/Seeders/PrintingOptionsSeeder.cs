...
public static class PrintingOptionsSeeder
{
    public static async Task SeedPrintingOptionsAsync(this IApplicationBuilder app)
    {
        ...
        var options = new List<PrintingOption>([
            PrintingOption.New(new LocalizedString("Без друку", "No print")),
            PrintingOption.New(new LocalizedString("1-3 кольори (Логотип)", "1-3 Colors (Logo)")),
            PrintingOption.New(new LocalizedString("Повноколірний друк (Патерн/Заливка)", "Full Color Print (Pattern/Fill)"))
        ]);
